...
.Konsept for publisering av data og hendelser - logisk dataflyt
image::../nab_referanse_arkitekturer_datautveksling_publisering/media/Konsept for publisering av data og hendelser.png[alt=Konsept for publisering av data og hendelser image]

Legg spesielt merke til:


* Figuren viser den logiske flyten av notifikasjoner og data. I en ren push-basert løsning vil dette tilsvare meldingsflyt, men i en pull-basert løsning er det datakonsumentene som tar initiativ til innhenting. 

* Notifikasjoner kan være "tynne" eller tykke", dvs. inneholde mer eller mindre data. Dersom alle aktuelle data er med i notifikasjonen, utgår behovet for å innhente supplerende data.
