...
Filtrering og minimering av notifikasjoner kan løses på ulike måter:

a. Tilbyder tilbyr ulike hendelseslister, der det i utgangspunktet er gjort et utvalg av det komplette utvalget innen tilbyders domene, basert på metadata og kriterier som tema eller konfidensialitet for hver hendelsesliste.

b. Tilbyder filtrerer innen hver hendelsesliste ut fra konsumentens spesifikasjon /spørring.

c. Tilbyder minimerer innholdet i hver notifikasjon ut fra konsumentens spesifikasjon /spørring. 

d. Konsumenten ignorerer irrelevante notifikasjoner ved å filtrere innlesingen eller forkaste notifikasjoner etter innlesing.
...
eNotifikasjon er en referansearkitektur for utveksling av notifikasjoner som er innrettet på å støtte opp under konsepter fra arkitekturmønsteret _Event Sourcing_; karakterisert ved at tilstanden til et _subjekt_ kan bestemmes gjennom en kronologisk sekvens av hendelser, ut fra et gitt øyeblikksbilde. Se under  <<litteraturhenvisninger-event-sourcing,litteraturhenvisninger om event sourcing>> for detaljer om Event Sourcing som arkitekturmønster.
...
Grunnleggende egenskaper ved eNotifikasjon som mønster, oppsummert:
