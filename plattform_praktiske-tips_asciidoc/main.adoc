...
. link:../kunnskap_verktøy_asciidoctor/[Mer om Asciidoctor].
. link:../plattform_praktiske-tips_github/[Praktiske tips om bruk av Github].
...
WARNING: For at innhold i mapper som ikke er direkte lenket til eller inkludert som asciidoc-filer skal komme med, må det plasseres en tom fil ved navn *.adocassets* i mappen. Denne filen bør ligge i alle media-mapper.

WARNING: Publiseringsløsningen krever at det ligger en fil ved navn *main.adoc* i alle mapper som skal kunne adresseres fra nettleser (tilsvarer index.html).


NOTE: Du kan følge med på publiseringsprosessen på
https://travis-ci.org/difi/nasjonal_arkitektur, der det også gi feilmeldinger og advarsler i tilknytning til aktuelle filer. Benytt dette aktivt for å sjekke dine publiserte filer for feil!  
...
Det eneste som behøvesfor å fjerne alle Eriks kommentarer fra visningen er å kommentere ut definisjonen av attributten (to skråstreker "//" først på linjen, dvs. "//:Eriks-kommentarer:").
