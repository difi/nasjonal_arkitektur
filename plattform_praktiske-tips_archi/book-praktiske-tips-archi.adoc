...
_NB: Om flere jobber på de samme modellene samtidig, kan det bli behov for litt kompliserte "merge-operasjoner". Om du opplever problemer med dette, er det fint om du melder fra via en _issue_ i GitHub (eller per e-post til nasjonalarkitektur@digdir.no).
...
=== Brukstilfelle 1: Integrer nasjonalt Archi-bibliotek i ditt eget Archi-bibliotek

1. Opprett ditt eget bibliotek (Archi-modell).

2. Få fatt i archi-filen til det nasjonale biblioteket. En måte å gjøre dette på, er via coArch plugin https://github.com/nasjonal-arkitektur/ac-nab-gjeldende, og så benytte temp.archimate som ligger under mappen .git (se over).

3. Importer archimate-filen fra det nasjonale biblioteket inn i ditt bibliotek.

4. Importer deretter oppdaterte versjoner av det nasjonale biblioteket.
+
TIP: Archi vil ikke slette verken folders, views eller elementer ved ny "import another model into selected model". Start derfor gjerne med å slette alle folders (og implisitt alle views) før ny import - så blir det ikke hengende igjen folders og views som skulle ha vært slettet. Etter ny import, kan en også slette ubrukte elementer og relasjoner (egne scripts for dette finnes).
...
a. Opprett et git repo med minst en fil. Dette kan f.eks gjøres slik: Logg inn på din org under github.com; klikk Add; oppgi repository name; sjekk av _Public_; sjekk av _Add a readme file_; klikk Add repository.

b. Via menyen under _Collaboration_: Add local model to workspace and publish. Du får da opp en dialog hvor du angir URL til det nyopprettede repoet.

c. Via menyen under _Collaboration_: Publish changes.
