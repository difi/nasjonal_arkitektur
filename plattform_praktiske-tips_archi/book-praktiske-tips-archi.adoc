...
== jArchi scripts fra Difi
=== Generelt
Difi har utviklet noen scripts for administrasjon av arkitekturbiblioteket. Disse scriptene er gjort i jArchi med bruk av Archi-s Scripting Plugin, og krever installasjon av  Archi-s Scripting Plugin for å kunne kjøres.

NOTE: Disse scriptene utvikles for å dekke løpende behov i påvente av at tilsvarende funksjonalitet (eventuelt) kommer i selve Archi-verktøyet. Kvaliteten på scriptene vil kunne variere, selv om de fleste er noenlunde grundig testet.

Produtdokumentasjon for jArchi finnes på  https://github.com/archimatetool/archi-scripting-plugin/wiki.
...
=== Kjøring av scipts - generelelt
Du kan  kjøre aktuelle scripts, enten ved å høyreklikke en _selection_ eller fra Scripts Manager (høyreklikk i scripts-listen eller klikk ikonet for _Run script_).

.Figur: Start av script ved å høyreklikke _selection_    
image:../plattform_praktiske-tips_archi/media/jArchi-script-selection.png[]
...
=== Introduksjon til funksjonalitet og praktisk bruk
Her gis en kort oppsummering på norsk. Mer utførlig dokumentasjon på engelsk er under utarbeidelse.
...
. *Enkel merge-funksjonalitet*
+
Et typisk problem når en skal vedlikeholde større modellbiblioteket, er en oppretter nye _konsepter_ med samme navn som eksisterende, i stedet for å gjenbruke eksisterende og bare opprette nye forekomster i hvert enkelt view. For å unngå dette, kan en høyreklikke 
