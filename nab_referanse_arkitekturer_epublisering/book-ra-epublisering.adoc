...
eNotifikasjon er et arkitekturmønster under området _deling av data ved publisering_ som omhandler publisering og innhenting av hendelsesdata, eller _notifikasjoner_, gjennom hendelseslister.

Hendelseslister gjøres tilgjengelig av datatilbyder via API-er. Tilgangsstyring og aksess av hendelsesdata gjøres samme måte som ved _deling av data på forespørsel_. Det samme gjelder for innhenting av tilleggsdata om hendelser, der dette er aktuelt.

Det som er spesifikt for eNotifikasjon, er måten å håndtere hendelseslister på, for henhoildsvis datatilbydere og datakonsumenter. 
