...
ePublisering er et arkitekturmønster under området _deling av data ved publisering_. Mønsteret forutsetter de samme kapabilitetene som eOppslag for oppsett og utveksling av data. Hendelseslister gjøres tilgjengelig som APIer på samme måte som andre oppslag og benytter tilsvarende mekanismer for tilgangsstyring. Det som  kommer i tillegg er kapabiliteter for å generere og tilby notifikasjon (tilbydere), samt å innhente og håndtere notifikasjoner (konsumenter). 
