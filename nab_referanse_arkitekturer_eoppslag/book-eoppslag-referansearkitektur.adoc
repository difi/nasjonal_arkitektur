...
// Test lenke: <<../nab_arkitekturlandskapr_strategisk-arkitektur_kapabilitetskart#Deling av data på forespørsel, Deling av data på forespørsel>>
...
eOppslag som referansearkitektur gir videre ingen begrensninger i forhold til  hva slags informasjon det gjelder, om det er strukturerte eller ustrukturerte data eller hva data skal brukes til. Tilbyder og konsument av data kan være på tvers av virksomheter eller innenfor samme virksomhet.
...
* Oppslag av i data innen en virksomhet underveis i en _tverrgående prosess_.


== Struktur på beskrivelsene
...
NOTE: Det er også publisert klikkbare Archimate-modeller under https://nasjonal-arkitektur.github.io/nab_modeller_html-hovedbibliotek/. De aktuelle modellene kan nås gjennom hyperlenking fra figurene her.

Det skilles mellom generiske arkitekturmønstre og løsningsmønstre. Først beskrives de generiske arkitekturmønstrene, uten kobling til spesifikke løsninger. Det gis deretter mer løsningsnære beskrivelser som inkluderer god praksis for bruk av _nasjonale fellesløsninger_.


De detaljerte beskrivelsene har separate underkapitler om klargjøring og operativ deling av data.

//å klargjøre for deling av data på forespørsel. Det er prosessene som må til for å tilgjengeliggjøre API-er, klienter og å sette opp tilganger både fra tilbyder og konsument sin side. Den andre grupperer de prosessene som inngår i den faktiske delingen av data på forespørsel.


Merk: Beskrivelsene som omhandler oppsett og bruk av tjenester tilknyttet tilgangsstyring, er ikke relevant for API-er som tilbyr åpent tilgjengelige data.
...
Tilbyder må kunne tilgjengeliggjøre data og dette er delt i to kapabiliteter. _Tilgjengeliggjøre API_ omhandler hvordan man registerer og setter opp tilganger. _Avgi forespurte data_ omhandler den operative delingen av data.


Tilsvarende er det tre detaljerte kapabiliteter tilknyttet konsument og det å innhente data. _Få tilgang til API_ består av prosesser for oppsett av tilgang, _Delegering av rettigheter til databehandler_ må gjøres dersom konsument benytter seg av en databehandler og _Slå opp data gjennom synkront API_ er det konsumenten må gjøre hver gang det gjøres et oppslag.
