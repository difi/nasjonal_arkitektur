...
Modellene beskrives først som generiske arkitekturmønstre uten kobling til spesifikke løsninger. Det gis deretter en mer løsningsnære beskrivelser som inkluderer god praksis for bruk av _nasjonale fellesløsninger_.
...
#Erik-kommentar: Dette kapitlet dreier seg om kapabilitetskartet, ikke eOppslag som sådan. Figur og tekst kan eventuelt inkluderes her, men "master" bør ligge i sammenheng med kapabilitetskartene. Dessuten kan det være et poeng å legge til en figur under kapitlet om _identifiserte_referansearkitekturer_ for å angi en mer eksplisitt kopling fra området _Deling av data på forespørsel_ til eOppslag referansearkitektur. Jeg ville forøvrig her flyttet "de hele" opp til kapitlet over "om eOppslag". Men NB: Kapabiliteten som heter "Slå opp data gjennom synkront API" bør omdøpes til "Slå opp data gjennom API".#
...
Tilgjengeliggjøring av API gjøres av tilbyder av data og er det tilbyder må gjennomføre for å gjøre et API synlig og tilgjengelig gjennom [red yellow-background]*fellesløsninger*. Registrering av konsumenters rettigheter og tilganger inngår også som et prosessteg. 
//erik: Nei, dette er ikke et prosessteg i samme prosess som de andre prosessboksene. Her vises snarere en verdikjede. Kan/bør vi fikse det nå? F.eks.gjennom å gruppere deto siste boksene og angi at det gjøres per konsument?
...
|Registrere API
|Prosess
|Prosess for tilbyder å registrere API i nasjonale [red yellow-background]*fellesløsninger* så det blir lettere tilgjengelig for konsumenter. Forutsetter at tilbyder har inngått avtale eller godtatt bruksvilkår for bruk av de nasjonale fellesløsningene.

// erik: registerere API i nasjonale fellesløsninger? Henrik: finne et annet begrep
...
|Forretningsprosess
|Prosessen med å finne eller få kjennskap til tilgjengelige API-er. Benytter tjeneste for å søke i [red yellow-background]*fellesløsninger*.
...
//erik: "Tokentjeneste" renavnes til
...
//erik: "Prosess med å motta forespørsler"er vel mer en event enn en prosess? Uansett er ordet prosess unødvendig he, Si heller bare Motta forespørsel...
//erik: "utlevere data" endres til avgi data? 
...
//erik: Dette ble litt kryptisk: "siden det er et tillitsforhold mellom partene"
|===
...
For enklere lesbarhet er laget med forretningstjenester utelatt fra modellene og mer løsningsnære applikasjonstjenester er benyttet direkte inn mot forretningsprosessene. Modellene viser dermed hvordan [red yellow-background]*fellesløsninger* i form av applikasjonskomponenter støtter de ulike forretningsprosessene gjennom applikasjonstjenester.
...
// erik:benutt "Felles API-katalog" somkomponentnavn
...
//Erik: Jeg savner noe om Altinn tilgangsstyring!??????????????????? 

|Registrere Open API Specification
//erik: Endre navn til "Registrere API i API-katalogen", og ta så heller eventuelt med info om at det erOpen API Specification som benyttes.
|Applikasjonstjeneste
|Tjeneste i API-katalogen for å registrere API. Bruk av tjenesten forutsetter at man har fått tildelt rettigheter til å gjøre dette på vegne av tilbyders virksomhet.
//erik: Hvem er "man" her? Det kan vel være tilbyder selv?
...
==== Få tilgang til API
For å få tilgang til et API sikret ved hjelp av [red yellow-background]*fellesløsninger* må konsumenten inngå avtale for bruk av Maskinporten og registrere den tekniske klienten som skal benytte løsningen.
...
erik:"tidligere tildelte rettigheter"?
