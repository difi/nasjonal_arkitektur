...
Beskrivelsen her er avgrenset til det å tilby og konsumere data gjennom synkrone kall mot APIer. Dette skal dekke både sikrede APIer hvor tilgangsstyring løses ved bruk av blant annet fellestjenester, og åpne APIer som tilbyr åpent tilgjengelige data. En stor del av beskrivelsene tar for seg oppsett og bruk av tjenester tilknyttet tilgangsstyring, men dette er ikke obligatorisk for APIer som tilbyr åpne data.
...
[red]**Sett inn forklarende figur**
...
Tilbyder må kunne tilggjengeliggjøre data og dette er delt i to kapabiliteter hvor den ene, registrering av API går ut på hvordan man registerer og setter opp tilganger. Den andre kapabilitetenm avhi forespurte data er det man gjøre når en konsument gjøre et kall mot det respektive APIet.

Tilsvarende er kapabilitetene tilknyttet konsument delt i to tre kapabiliteter hvor to går inn på oppsett før bruk og den tredje, slå opp data gjennom et API er det man må gjøre hver gang det gjøres et oppslag.
...
I dette kapittelet detaljeres kapabilitetene som inngår i deling av data på forespørsel. Modellen består av tre nivåer. I midten er det spesifisert noen prosessteg som realiserer kapabiliteten. Det vil si at kan du gjennomføre prosesstegene innehar du evnen kapabiliten beskriver.
...
image:./media/forklaring prosess.png[alt="Bilde mangler", width=250]
...
|Prosess
|Prosess for å tildele tilganger til de virksomheter som har et gyldig behandlingsgrunnlag og som har inngått avtale om tilgang til data.
...
===== Avgi forespurte data
Modellen under detaljerer kapabiliteten "Avgi forespurte data". 
...
===== Få tilgang til API
Modellen under detaljerer kapabiliteten "Få tilgang til API".
...
|Forretningsprosess
|Prosess for konsument å registerere den klienten som skal ha tilgang til APIet. Dette forutsetter at tilbyder har gitt konsument tilgang.

|Leverandør registrerer klient med tildelt tilgang
|Forretningsprosess
|Prosess for leverandør til konsument å registerere den klienten som skal ha tilgang til APIet. Dette forutsetter at tilbyder har gitt konsument tilgang og at leverandør har blitt delegert rettigheter som databehandler på vegne av konsument.
...
===== Delegering av rettigheter til databehandler
Modellen under detaljerer kapabiliteten "Delegering av rettigheter til databehandler".
...
===== Slå opp data gjennom et API
Modellen under detaljerer kapabiliteten "Slå opp data gjennom et API".
